class PerformancesController < ApplicationController
  ...
  def index
    @id_p = params[:id_p].to_s
    if (@id_p == "true")
      @performances = Performance.all.select{|x| !x.invoiced}
    else
  ...
  private
    ...
    def performance_params
      params.require(:performance).permit(:client_id, :description, :date, :duration, :invoiced, :rate_id)
    end
end
